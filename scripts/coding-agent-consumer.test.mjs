...
const devPackages = ["eaon-client", "eaon-protocol", "eaon-server"].map((name) => `@eaonlabs/${name}`);
...
function createFixture(t, { importServer = false, declareServer = false } = {}) {
	...
	const packages = [codingAgentName, "@eaonlabs/chord", ...devPackages].map((name) => ({
		name,
...
${importServer ? 'import "@eaonlabs/eaon-server";' : ""}
import { marker } from "@eaonlabs/chord";
...
test("fails when the SDK imports an undeclared server despite a working CLI", (t) => {
	...
	assert.throws(() => smokeTestCodingAgentConsumer(directory), /Cannot find package '@eaonlabs\/eaon-server'/);
});
