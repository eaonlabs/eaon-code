...
import { readFacetBundleManifest } from "@eaonlabs/chord/node";
...
describe("server-selected presentation facets", () => {
	...
	test("builds conventional plugin entries into the server-owned plugin cache", async () => {
		...
		await writeFile(
			...
			`${JSON.stringify({
				name: "@eaonlabs/test-plugin",
				version: "1.0.0",
				peerDependencies: {
					"@eaonlabs/chord": "^0.84.4",
					"@eaonlabs/eaon-code": "^0.84.4",
				},
			})}\n`,
		);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@eaonlabs/chord"; import { SlashCommands } from "@eaonlabs/eaon-code/experimental/plugin"; export default defineFacet({ id: "built-a", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@eaonlabs/chord"; import { SlashCommands } from "@eaonlabs/eaon-code/experimental/plugin"; export default defineFacet({ id: "built-b", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		await Promise.all([
			writeFile(
				...
				`${JSON.stringify({
					name: "@eaonlabs/second-test-plugin",
					version: "1.0.0",
					peerDependencies: { "@eaonlabs/chord": "^0.84.4" },
				})}\n`,
			),
			writeFile(
				join(secondPackagePath, "src", "tui.ts"),
				'import { defineFacet } from "@eaonlabs/chord"; export default defineFacet({ id: "second-built", setup() {} });\n',
			),
		]);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@eaonlabs/chord"; import { SlashCommands } from "@eaonlabs/eaon-code/experimental/plugin"; export default defineFacet({ id: "built-c", setup(env) { env.use(SlashCommands); } });\n',
		);
	...
	test("builds the example plugin package without a package-owned build script", async () => {
		...
		expect(manifest.plugin).toEqual({ id: "@eaonlabs/eaon-example-plugin", version: "1.0.0" });
		...
		expect(loaded.facets.map(({ id }) => id)).toEqual(["@eaonlabs/eaon-example-plugin/tui"]);
