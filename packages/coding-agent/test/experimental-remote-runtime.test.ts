...
import { type Context, createFacetHost, defineFacet, defineService } from "@eaonlabs/chord";
	...
	test("loads conventional Session facets from multiple configured plugin packages", async () => {
		...
		await Promise.all([
			writeFile(
				...
				`${JSON.stringify({
					name: "@eaonlabs/second-session-plugin",
					version: "1.0.0",
			...
			writeFile(
				...
				'import { defineFacet, defineService } from "@eaonlabs/chord"; const Service = defineService("test.second-plugin"); export default defineFacet({ id: "second-session-plugin", setup(env) { env.provide(Service, { async read() { return "second"; } }); } });\n',
			),
		]);
		const runtime = await startServer({ ...sessionWorkerModel, directory });
		servers.add(runtime);
