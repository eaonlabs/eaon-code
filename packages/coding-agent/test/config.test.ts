...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	const scopeDir = join(root, "@eaonlabs");
	const packageDir = join(scopeDir, "eaon-code");
...
function createPnpmGlobalInstall(): { root: string; packageDir: string } {
	...
	const packageDir = join(root, "@mariozechner", "eaon-code");
	mkdirSync(packageDir, { recursive: true });
...
function createYarnGlobalInstall(): { globalDir: string; packageDir: string } {
	...
	const packageDir = join(globalDir, "node_modules", "@mariozechner", "eaon-code");
	mkdirSync(packageDir, { recursive: true });
	mkdirSync(binDir, { recursive: true });
	...
	process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
	process.env.PI_PACKAGE_DIR = packageDir;
	setExecPath(join(globalDir, ".yarn", "@mariozechner", "eaon-code", "dist", "cli.js"));
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@eaonlabs+eaon-code@0.67.68\\node_modules\\@eaonlabs\\eaon-code\\dist\\cli.js",
		);

		expect(detectInstallMethod()).toBe("pnpm");
		expect(getUpdateInstruction("@eaonlabs/eaon-code")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @eaonlabs/eaon-code",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@eaonlabs/eaon-code");
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@eaonlabs/eaon-code", undefined, {
			packageName: "@eaonlabs/eaon-code",
			installSpec: "@eaonlabs/eaon-code@1.2.3",
		});
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@eaonlabs/eaon-code@1.2.3",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @eaonlabs/eaon-code@1.2.3`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@eaonlabs/eaon-code", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @eaonlabs/eaon-code`,
		);
	});

	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\@eaonlabs\\eaon-code";
		process.env.PI_PACKAGE_DIR = packageDir;
		setExecPath(`${packageDir}\\dist\\cli.js`);

		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@eaonlabs/eaon-code")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @eaonlabs/eaon-code",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@eaonlabs/eaon-code"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @eaonlabs/eaon-code",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@eaonlabs/eaon-code";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@eaonlabs", "eaon-code");
