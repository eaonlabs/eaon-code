...
Eaon Code supports subscription-based providers via OAuth and API key providers via environment variables or auth file. Built-in catalogs ship with Eaon Code; configured providers may refresh newer catalogs and cache them in `~/.eaon/agent/models-store.json` for offline use.
...
Use `/logout` to clear credentials. Tokens are stored in `~/.eaon/agent/auth.json` and auto-refresh when expired. OpenRouter instead mints a user-controlled API key that does not expire automatically.
...
- Run `/login meta` and follow the device authorization prompt
- Or choose **Use an API key** and set `META_API_KEY`
...
Store credentials in `~/.eaon/agent/auth.json`:
...
Use this when Eaon Code should use different provider settings than the project shell environment.
...
```bash
eaon-code --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
eaon-code --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
eaon-code --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
For normal Eaon Code usage, prefer unified billing or stored BYOK. Inline BYOK requires configuring an additional upstream `Authorization` header for the Cloudflare AI Gateway provider, for example via a `models.json` provider/model override.
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
eaon-code --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```

Eaon Code automatically sets `x-session-affinity` for [prefix caching](https://developers.cloudflare.com/workers-ai/features/prompt-caching/) discounts.
...
Eaon Code supports the llama.cpp router server. Configure it with `/login llama.cpp`, manage loaded models with `/llama`, and select a loaded model with `/model`.
