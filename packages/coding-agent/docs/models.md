...
Add custom providers and models (Ollama, vLLM, LM Studio, proxies) via `~/.eaon/agent/models.json`.
...
The `apiKey` value is a placeholder because Ollama ignores it. Eaon Code still treats models as requiring auth before they appear in `/model`, so keyless local servers should keep a dummy value, save a key for that provider with `/login`, or pass `--api-key` when selecting the model.

Some OpenAI-compatible servers do not understand the `developer` role used for reasoning-capable models. For those providers, set `compat.supportsDeveloperRole` to `false` so Eaon Code sends the system prompt as a `system` message instead. If the server also does not support `reasoning_effort`, set `compat.supportsReasoningEffort` to `false` too.
...
| `noAuth` | Set `true` for endpoints that require no credentials. Models are available without `/login`, and no bearer token is sent. |
...
For providers with `models`, non-built-in provider configs need `baseUrl` and an `api` value at either provider or model level. `apiKey` is not required to load the file: models become available when auth is configured through `/login`/`auth.json`, CLI `--api-key`, or provider `apiKey`. If the endpoint requires no credentials, set `noAuth: true` to make its models available without login. Otherwise, models without configured auth stay unavailable in `/model` and `--list-models`.
...
For `models.json`, shell commands are resolved at request time. Eaon Code intentionally does not apply built-in TTL, stale reuse, or recovery logic for arbitrary commands. Different commands need different caching and failure strategies, and Eaon Code cannot infer the right one.
...
| `thinkingLevelMap` | No | omitted | Maps Eaon Code thinking levels to provider values and marks unsupported levels (see below) |
...
`samplingParams` is a free-form object merged verbatim into every request body for the model, after the fields Eaon Code sets itself, so its keys win. Use it to send sampling parameters Eaon Code does not model — including server-specific ones like llama.cpp's `min_p` or vLLM's `top_k`:
...
Use `thinkingLevelMap` on a model to describe model-specific thinking controls. Keys are Eaon Code thinking levels: `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`. Maps may contain holes; for example, a model can expose `high` and `max` without exposing `xhigh`.
...
| `supportsMidConvoEffort` | Whether the exact Claude model transport supports per-turn effort system messages and thinking binding controls. Eaon Code persists native effort levels and always sends `drop_block` when enabled. Default: `false`. |
...
| `supportsFinishReason` | Whether streamed responses include `finish_reason`. When `false`, Eaon Code infers `stop` or `toolUse` when the stream ends. Default: `true`. |
...
| `chatTemplateKwargs` | `chat_template_kwargs` values for `thinkingFormat: "chat-template"`; use `{ "$var": "thinking.enabled" }`, `{ "$var": "thinking.effort" }`, or `{ "$var": "thinking.budget" }` for Eaon Code-controlled thinking values |
| `chatTemplateArgs` | `chat_template_args` values for `thinkingFormat: "baseten"`; use `{ "$var": "thinking.enabled" }`, `{ "$var": "thinking.effort" }`, or `{ "$var": "thinking.budget" }` for Eaon Code-controlled thinking values |
