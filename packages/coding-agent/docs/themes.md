> Eaon Code can create themes. Ask it to build one for your setup.
...
Eaon Code loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.eaon/agent/themes/*.json`
- Project: `.eaon/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `eaon.themes` entries in `package.json`
...
On first run, Eaon Code detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.eaon/agent/themes
vim ~/.eaon/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/eaonlabs/eaon-code/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, Eaon Code reloads it automatically for immediate visual feedback.
...
Eaon Code uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Eaon Code falls back to the nearest approximation.
