...
Eaon Code saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.eaon/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.
...
eaon-code --session <path|id> # Use a specific session file or partial session ID
eaon-code --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `eaon-code -r` opens the same picker at startup.
...
When available, Eaon Code uses the `trash` CLI for deletion instead of permanently removing files.
...
```bash
eaon-code --name "Refactor auth module"
eaon-code --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `eaon-code -r`.
...
When `/tree` switches away from one branch to another, Eaon Code can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
