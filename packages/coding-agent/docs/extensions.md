> Eaon Code can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend Eaon Code's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.

> **Placement for /reload:** Put extensions in `~/.eaon/agent/extensions/` (global) or `.eaon/extensions/` (project-local) for auto-discovery. Use `eaon-code -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.

**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `eaon.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `eaon.registerCommand()`
- **Session persistence** - Store state that survives restarts via `eaon.appendEntry()`
...
Create `~/.eaon/agent/extensions/my-extension.ts`:
...
export default function (eaon: ExtensionAPI) {
  ...
  eaon.on("tool_call", async (event, ctx) => {
...
Extensions are auto-discovered from trusted locations. Project-local `.eaon/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.eaon/agent/extensions/*.ts` | Global (all projects) |
| `~/.eaon/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.eaon/extensions/*.ts` | Project-local |
| `.eaon/extensions/*/index.ts` | Project-local (subdirectory) |
...
To share extensions via npm or git as Eaon Code packages, see [packages.md](packages.md).

## Available Imports

| Package | Purpose |
|---------|---------|
| `@eaonlabs/eaon-code` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@eaonlabs/eaon-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@eaonlabs/eaon-tui` | TUI components for custom rendering |

npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For distributed Eaon Code packages installed with `eaon-code install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
export default function (eaon: ExtensionAPI) {
  // Subscribe to events
  eaon.on("event_name", async (event, ctx) => {
  ...
  eaon.registerTool({ ... });
  eaon.registerCommand("name", { ... });
  eaon.registerShortcut("ctrl+x", { ... });
  eaon.registerFlag("my-flag", { ... });
}
```

Extensions are loaded via [jiti](https://github.com/unjs/jiti), so TypeScript works without compilation.

If the factory returns a `Promise`, Eaon Code awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `eaon.registerProvider()` are flushed.
...
export default async function (eaon: ExtensionAPI) {
  ...
  eaon.registerProvider("local-openai", {
...
This pattern makes the fetched models available during normal startup and to `eaon-code --list-models`.
...
thinking level changes (settings, keybinding, eaon.setThinkingLevel())
...
Fired before Eaon Code decides whether to trust a project with dynamic configs (`.eaon` (or legacy `.pi`) or `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.

```typescript
eaon.on("project_trust", async (event, ctx) => {
...
A `project_trust` handler must return `{ trusted: "yes" | "no" | "undecided" }`. A user/global or CLI extension that returns `"yes"` or `"no"` owns the decision; the first yes/no decision wins and suppresses the built-in trust prompt. Use `remember: true` to persist a yes/no decision; otherwise it applies only to the current process. Return `"undecided"` to let later handlers or the built-in trust flow decide. Check `ctx.hasUI` before prompting. If no handler returns yes/no, normal trust resolution continues: saved `trust.json` decisions apply first, then `defaultProjectTrust` controls whether Eaon Code asks, trusts, or declines by default.
...
```typescript
eaon.on("resources_discover", async (event, _ctx) => {
  // event.cwd - current working directory
...
```typescript
eaon.on("session_start", async (event, ctx) => {
...
Fired when the current session display name is set via `/name`, RPC, or `eaon.setSessionName()`.

```typescript
eaon.on("session_info_changed", async (event, ctx) => {
...
```typescript
eaon.on("session_before_switch", async (event, ctx) => {
...
After a successful switch or new-session action, Eaon Code emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
```typescript
eaon.on("session_before_fork", async (event, ctx) => {
...
After a successful fork or clone, Eaon Code emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
```typescript
eaon.on("session_before_compact", async (event, ctx) => {
...
eaon.on("session_compact", async (event, ctx) => {
...
eaon.on("session_compact_failed", async (event, ctx) => {
  // event.reason - "manual" (/compact), "threshold", or "overflow"
...
```typescript
eaon.on("session_before_tree", async (event, ctx) => {
...
eaon.on("session_tree", async (event, ctx) => {
...
```typescript
eaon.on("session_shutdown", async (event, ctx) => {
...
```typescript
eaon.on("before_agent_start", async (event, ctx) => {
...
The `systemPromptOptions` field gives extensions access to the same structured data Eaon Code uses to build the system prompt. This lets you inspect what Eaon Code has loaded — custom prompts, guidelines, tool snippets, context files, skills — without re-discovering resources or re-parsing flags. Use it when your extension needs to make deep, informed changes to the system prompt while respecting user-provided configuration.

Inside `before_agent_start`, `event.systemPrompt` and `ctx.getSystemPrompt()` both reflect the chained system prompt as of the current handler. Later `before_agent_start` handlers can still modify it again.

#### agent_start / agent_end / agent_settled

`agent_start` fires when a low-level agent run begins. `agent_end` fires when that run ends, but Eaon Code may still auto-retry, auto-compact and retry, or continue with queued follow-up messages. Use `agent_settled` for status integrations that need to know Eaon Code will not continue running automatically.

```typescript
eaon.on("agent_start", async (_event, ctx) => {});

eaon.on("agent_end", async (event, ctx) => {
  // event.messages - messages from this low-level run
});

eaon.on("agent_settled", async (_event, ctx) => {
...
```typescript
eaon.on("ui_prompt_start", async (event, ctx) => {
  // event.reason === "ui_prompt"
  // event.kind: "select" | "confirm" | "input" | "editor" | "custom"
  // event.title: prompt title when available
});

eaon.on("ui_prompt_end", async (event, ctx) => {
  // Eaon Code is no longer waiting on that UI prompt span.
});
```
...
```typescript
eaon.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

eaon.on("turn_end", async (event, ctx) => {
...
```typescript
eaon.on("message_start", async (event, ctx) => {
  // event.message
});

eaon.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

eaon.on("message_end", async (event, ctx) => {
...
```typescript
eaon.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

eaon.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

eaon.on("tool_execution_end", async (event, ctx) => {
...
```typescript
eaon.on("context", async (event, ctx) => {
...
```typescript
eaon.on("before_provider_headers", (event, ctx) => {
  // Add or override — e.g. a session id for gateway tracing/attribution
  event.headers["x-session-id"] = ctx.sessionManager.getSessionId();

  // Drop a tracking header Eaon Code adds for this call
...
This hook can rewrite provider-level system instructions or remove them entirely. Those payload-level changes are not reflected by `ctx.getSystemPrompt()`, which reports Eaon Code's system prompt string rather than the final serialized provider payload.

```typescript
eaon.on("before_provider_request", (event, ctx) => {
...
```typescript
eaon.on("after_provider_response", (event, ctx) => {
...
```typescript
eaon.on("model_select", async (event, ctx) => {
...
```typescript
eaon.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `eaon.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
Before `tool_call` runs, Eaon Code waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "@eaonlabs/eaon-code";
...
```typescript
import { isBashToolResult } from "@eaonlabs/eaon-code";

eaon.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "@eaonlabs/eaon-code";

eaon.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap Eaon Code's built-in local bash backend
...
Use `CONFIG_DIR_NAME` instead of hardcoding `.eaon` when constructing project-local config paths. Rebranded distributions can use a different config directory name.

```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@eaonlabs/eaon-code";
...
Use `ctx.modelRegistry.streamSimple(model, context, options)` for provider-neutral options such as `reasoning`, or `stream()` for API-specific options. Both use configured providers and resolve authentication, including for providers registered with `eaon.registerProvider()`. Use these instead of `@eaonlabs/eaon-ai/compat` streaming functions, which cannot see extension provider registrations.
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while Eaon Code is idle.
...
Control flow helpers. `ctx.isIdle()` is false while Eaon Code is processing an agent run, automatic retry, auto-compaction retry, or queued continuation.

### ctx.shutdown()

Request a graceful shutdown of Eaon Code.
...
Returns Eaon Code's current system prompt string.
...
```typescript
eaon.on("before_agent_start", (event, ctx) => {
...
Returns the base inputs Eaon Code currently uses to build the system prompt.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `eaon-code` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "@eaonlabs/eaon-code";

eaon.registerCommand("switch", {
...
- Captured old `eaon-code` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
eaon.registerCommand("reload-runtime", {
...
export default function (eaon: ExtensionAPI) {
  ...
  eaon.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      eaon.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
### eaon.on(event, handler)

Subscribe to events. See [Events](#events) for event types and return values.

### eaon.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](#custom-tools) for full details.

`eaon.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `eaon.getAllTools()` and are callable by the LLM without `/reload`.

Use `eaon.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@eaonlabs/eaon-ai";
...
### eaon.sendMessage(message, options?)

Inject a custom message into the session. Custom messages participate in LLM context. For durable TUI-only content that should not be sent to the LLM, use [`eaon.appendEntry()`](#piappendentrycustomtype-data) with [`eaon.registerEntryRenderer()`](#piregisterentryrenderercustomtype-renderer).
...
### eaon.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
eaon.sendUserMessage("What is 2+2?");

// With content array (text + images)
eaon.sendUserMessage([
  { type: "text", text: "Describe this image:" },
  { type: "image", source: { type: "base64", mediaType: "image/png", data: "..." } },
]);

// During streaming - must specify delivery mode
eaon.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
eaon.sendUserMessage("And then summarize", { deliverAs: "followUp" });

// Opt in to extension command dispatch and skill/prompt template expansion
eaon.sendUserMessage("/review src/index.ts", { expandPromptTemplates: true });
```
...
### eaon.appendEntry(customType, data?)

Persist extension data. Custom entries do NOT participate in LLM context. In interactive mode, they can also render inside the chat transcript when paired with `eaon.registerEntryRenderer()`.

```typescript
eaon.appendEntry("my-state", { count: 42 });
eaon.appendEntry("status-card", { title: "Indexed files", count: 17 });
...
### eaon.setSessionName(name)

Set the session display name (shown in session selector instead of first message).

```typescript
eaon.setSessionName("Refactor auth module");
```
...
```typescript
const name = eaon.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### eaon.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
eaon.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
eaon.setLabel(entryId, undefined);
...
### eaon.registerCommand(name, options)

Register a command.

If multiple extensions register the same command name, Eaon Code keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.

```typescript
eaon.registerCommand("stats", {
...
```typescript
import type { AutocompleteItem } from "@eaonlabs/eaon-tui";

eaon.registerCommand("deploy", {
...
```typescript
const commands = eaon.getCommands();
...
### eaon.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for custom messages with your `customType`. Custom messages are created with `eaon.sendMessage()` and participate in LLM context. See [Custom UI](#custom-ui).

### eaon.registerMarkdownTransformer(transformer)

Register a transformer for the Markdown in normal user text, assistant text, and thinking blocks. Transformers run in extension load order, and each transformer receives the Markdown returned by the previous transformer. After the chain finishes, Eaon Code renders the transformed content with its built-in renderer.
...
```typescript
eaon.registerMarkdownTransformer((markdown, { messageType, isStreaming }) => {
  if (isStreaming || messageType === "assistant-thinking") return markdown;
  return markdown.replaceAll("-->", "→");
});
```

If a transformer throws, Eaon Code keeps the Markdown produced so far and continues with the next transformer. The hook is display-only: the original message remains unchanged in the session and model context. It runs for new user messages, assistant streaming updates, restored session messages, and terminal width changes, so transformers should remain synchronous and inexpensive.

### eaon.registerEntryRenderer(customType, renderer)

Register a custom TUI renderer for custom entries with your `customType`. Custom entries are created with `eaon.appendEntry()` and do not participate in LLM context.

```typescript
import { Box, Text } from "@eaonlabs/eaon-tui";

eaon.registerEntryRenderer("status-card", (entry, { expanded }, theme) => {
...
### eaon.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
eaon.registerShortcut("ctrl+shift+p", {
...
### eaon.registerFlag(name, options)

Register a CLI flag.

```typescript
eaon.registerFlag("plan", {
...
// Check value
if (eaon.getFlag("plan")) {
  // Plan mode enabled
}
```

### eaon.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await eaon.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### eaon.getActiveTools() / pi.getAllTools() / pi.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools. `eaon.getActiveTools()` returns the active tool names as `string[]`; `eaon.getAllTools()` returns metadata for all configured tools.

```typescript
const active = eaon.getActiveTools(); // ["read", "bash", ...]
const all = eaon.getAllTools();
// all = [{
//   name: "read",
...
eaon.setActiveTools([...new Set([...active, "my_custom_tool"])]); // Keep current tools and enable my_custom_tool
eaon.setActiveTools(["read", "bash"]); // Switch to read-only
```

`eaon.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await eaon.setModel(model);
  if (!success) {
...
### eaon.getThinkingLevel() / pi.setThinkingLevel(level)
...
`eaon.setThinkingLevel()` changes the thinking level for the current session. The change is recorded in session history and restored when that session is resumed, but it does not change the configured default used by new sessions.

```typescript
const current = eaon.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"
eaon.setThinkingLevel("high");
```
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@eaonlabs/eaon-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@eaonlabs/eaon-ai";
...
eaon.registerProvider(provider);

// Register a new provider with custom models
eaon.registerProvider("my-proxy", {
  name: "My Proxy",
...
eaon.registerProvider("llama.cpp", {
  baseUrl: "http://localhost:8080/v1",
  apiKey: "local",
  api: "openai-completions",
...
eaon.registerProvider("anthropic", {
  baseUrl: "https://proxy.example.com"
});

// Register provider with OAuth support for /login
eaon.registerProvider("corporate-ai", {
...
The object form accepts a complete Eaon AI `Provider`, including native `auth`, `getModels`, `refreshModels`, `filterModels`, `stream`, and `streamSimple` behavior.
...
```typescript
eaon.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    eaon.unregisterProvider("my-proxy");
  },
});
```
...
```typescript
import { withFileMutationQueue } from "@eaonlabs/eaon-code";
...
import { Type } from "typebox";
import { StringEnum } from "@eaonlabs/eaon-ai";
import { Text } from "@eaonlabs/eaon-tui";
...
eaon.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via eaon.exec (captured from extension closure)
    const result = await eaon.exec("some-command", [], { signal });
...
**Usage accounting:** If a tool makes nested LLM calls, return their combined `Usage` as `usage`. Eaon Code persists it on the tool result and includes it in footer, `/session`, and RPC session totals. `tool_result` handlers can inspect or replace this value.
...
**Important:** Use `StringEnum` from `@eaonlabs/eaon-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when Eaon Code resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
eaon-code -e ./tool-override.ts
```

Alternatively, use `--no-builtin-tools` to start without any built-in tools while keeping extension tools enabled:
```bash
# No built-in tools, only extension tools
eaon-code --no-builtin-tools -e ./my-extension.ts
```
...
- [read.ts](https://github.com/eaonlabs/eaon-code/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/eaonlabs/eaon-code/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [powershell.ts](https://github.com/eaonlabs/eaon-code/blob/main/packages/coding-agent/src/core/tools/powershell.ts) - `PowerShellToolDetails`
- [edit.ts](https://github.com/eaonlabs/eaon-code/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/eaonlabs/eaon-code/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/eaonlabs/eaon-code/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/eaonlabs/eaon-code/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/eaonlabs/eaon-code/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@eaonlabs/eaon-code";
...
For `user_bash`, extensions can reuse Eaon Code's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.

The `bash` and `powershell` tools also support a spawn hook to adjust the command, cwd, or env before execution:

```typescript
import { createBashTool } from "@eaonlabs/eaon-code";
...
export default function (eaon: ExtensionAPI) {
  ...
  eaon.registerTool({ name: "db_connect", ... });
  eaon.registerTool({ name: "db_query", ... });
  eaon.registerTool({ name: "db_close", ... });

  eaon.on("session_shutdown", async () => {
    connection?.close();
  });
}
```
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/eaonlabs/eaon-code/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@eaonlabs/eaon-code";
...
Extensions can register many tools while keeping only a small initial set active. A tool can then add more tools with `eaon.setActiveTools()` during execution. Eaon Code detects purely additive changes, records the newly available tool names on that tool result, and applies the updated active set before the next model request.
...
1. Register every tool with `eaon.registerTool()` so it appears in `eaon.getAllTools()`.
2. Keep loader tools, such as `search_tools`, active and leave searchable tools inactive.
3. During loader execution, call `eaon.setActiveTools([...currentTools, ...matchingTools])`. The change must be additive: do not remove currently active tools in the same call.
4. Eaon Code records which tools were added on the loader's tool result.
5. Before the next model response, Eaon Code exposes the added definitions using native deferred loading when supported, or the normal active tool list otherwise.

You do not need to return provider-specific tool references or mark the loader as a special search tool. The active-tool change is the signal. Names passed to `eaon.setActiveTools()` must already be registered; unknown names are ignored.
...
- **OpenAI**
  ...
  - **Native representation:** Eaon Code adds completed client `tool_search_call` and `tool_search_output` items at the load point.
...
For all other models and providers, dynamic activation still works: Eaon Code sends the complete current active tool list normally on the next request. The model can call the newly activated tools, but adding their definitions may invalidate the provider's cached prompt prefix.

Eaon Code also uses this safe fallback when the active set is not purely additive, such as replacing one group of tools with another. Tool removals therefore work, but they do not use deferred loading.
...
export default function (eaon: ExtensionAPI) {
  ...
  eaon.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const matches = eaon.getAllTools()
      ...
      const active = eaon.getActiveTools();
      const added = matches.filter((name) => !active.includes(name));
      eaon.setActiveTools([...new Set([...active, ...added])]);
  ...
  eaon.on("session_start", () => {
    // Keep searchable tools registered but initially inactive. Preserve built-ins
    // and tools owned by other extensions, and keep the loader itself active.
    const initialTools = eaon.getActiveTools().filter(
      (name) => !SEARCHABLE_TOOL_NAMES.has(name),
    );
    eaon.setActiveTools([...new Set([...initialTools, "search_tools"])]);
  });
}
```
...
// Terminal title
ctx.ui.setTitle("eaon-code - my-project");
...
```typescript
import { Text, Component } from "@eaonlabs/eaon-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@eaonlabs/eaon-code";
import { matchesKey } from "@eaonlabs/eaon-tui";
...
eaon.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `eaon.sendMessage()`:
...
```typescript
eaon.registerEntryRenderer("my-card", (entry, options, theme) => {
  return new Text(theme.fg("accent", JSON.stringify(entry.data)));
});

eaon.appendEntry("my-card", { status: "done" });
```
...
```typescript
import { highlightCode, getLanguageFromPath } from "@eaonlabs/eaon-code";
...
| `event-bus.ts` | Inter-extension events | `eaon.events` |
| **Session Metadata** |||
