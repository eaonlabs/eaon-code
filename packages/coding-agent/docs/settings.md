...
Eaon Code uses JSON settings files with project settings overriding global settings.

| Location | Scope |
|----------|-------|
| `~/.eaon/agent/settings.json` | Global (all projects) |
| `.eaon/settings.json` | Project (current directory) |
...
For VS Code, include `--wait` so Eaon Code resumes after the editor exits:
...
Keep `retry.provider.maxRetries` at `0` unless provider-level retries are explicitly needed. Setting it above `0` can make SDK/provider retries handle out-of-usage-limit errors before Eaon Code sees them, which may block the agent until the provider quota resets in some circumstances.
...
`npmCommand` is used for all npm package-manager operations, including installs, uninstalls, and dependency installs inside git packages. User-scoped npm packages install under `~/.eaon/agent/npm/`; project-scoped npm packages install under `.eaon/npm/`. Use argv-style entries exactly as the process should be launched. When `npmCommand` is configured, git package dependency installs use plain `install` to avoid npm-specific flags in wrappers or alternate package managers.
...
Paths in `~/.eaon/agent/settings.json` resolve relative to `~/.eaon/agent`. Paths in `.eaon/settings.json` resolve relative to `.eaon`. Absolute paths and `~` are supported.
...
```json
{
  "packages": ["my-skills-package", "@org/my-extension"]
}
```
...
```json
{
  ...
  "packages": ["my-skills-package"]
}
```

## Project Overrides

Project settings (`.eaon/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.eaon/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .eaon/settings.json (project)
{
