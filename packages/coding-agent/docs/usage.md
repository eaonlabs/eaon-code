...
On Windows Terminal, Alt+Enter is fullscreen by default. Remap it as described in [Terminal setup](terminal-setup.md) if you want Eaon Code to receive the shortcut.

Configure delivery in [Settings](settings.md) with `steeringMode` and `followUpMode`.

## Sessions

Sessions are saved automatically to `~/.eaon/agent/sessions/`, organized by working directory.

```bash
eaon-code -c                  # Continue most recent session
eaon-code -r                  # Browse and select a session
eaon-code --no-session        # Ephemeral mode; do not save
eaon-code --name "my task"    # Set session display name at startup
eaon-code --session <path|id> # Use a specific session file or session ID
eaon-code --fork <path|id>    # Fork a session into a new session file
```
...
- `~/.eaon/agent/AGENTS.md` for global instructions
- parent directories, walking up from the current working directory
- the current directory

If a directory contains `AGENTS.override.md`, Eaon Code loads it instead of `AGENTS.md` or `CLAUDE.md` from that directory. Context files from other directories still layer normally.
...
- `.eaon/SYSTEM.md` for a project
- `~/.eaon/agent/SYSTEM.md` globally

Append to the default prompt without replacing it with `APPEND_SYSTEM.md` in either location.

### Project Trust

On interactive startup, Eaon Code asks before trusting a project folder that contains project-local settings, resources, or project `.agents/skills` and has no saved decision for the folder or a parent folder in `~/.eaon/agent/trust.json`. Trusting a project allows Eaon Code to load `.eaon/settings.json` and resources in `.eaon/` (or an existing legacy `.pi/` directory), install missing project packages, and execute project extensions.
...
eaon-code update [source|self]   # Update Eaon Code only, or one package source
eaon-code update --all              # Update Eaon Code and packages; reconcile pinned git refs
eaon-code update --extensions       # Update packages only; reconcile pinned git refs
...
These commands manage Eaon Code packages and `eaon-code update` can update the Eaon Code CLI installation. To uninstall Eaon Code itself, see [Quickstart](quickstart.md#uninstall). `eaon-code config` and project package commands accept `--approve`/`--no-approve` to trust or ignore project-local settings for one command. `eaon-code update` never prompts for project trust.

See [Eaon Code Packages](packages.md) for package sources and security notes.
...
```bash
eaon-code --no-extensions -e ./my-extension.ts
```
...
| `--tui-mode <mode>` | Override saved TUI mode: `regular` or `fullscreen` (fullscreen on first install) |
...
In `fullscreen` mode, the transcript scrolls inside the terminal viewport while queued messages, working status, extension widgets, editor, and footer remain fixed at the bottom. Mouse/trackpad input scrolls the region under the pointer; keyboard viewport actions always remain available. Inline images work in terminals that support the Kitty graphics protocol, including Kitty and Ghostty. In iTerm2 they render as text placeholders because its inline-image protocol cannot delete or crop placements during application-owned scrolling. In `regular` mode, Eaon Code uses the main screen and terminal-owned scrollback, and iTerm2 inline images continue to render normally. See [Terminal setup](terminal-setup.md) for terminal-specific settings and workarounds.
...
Eaon Code keeps the core small and pushes workflow-specific behavior into extensions, skills, prompt templates, and packages.
