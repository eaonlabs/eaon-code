...
```bash
npm install @eaonlabs/eaon-code
```
...
```typescript
import { createAgentSession, SessionManager } from "@eaonlabs/eaon-code";
...
The `Agent` class (from `@eaonlabs/eaon-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
```typescript
import { getModel } from "@eaonlabs/eaon-ai";
import { ModelRuntime } from "@eaonlabs/eaon-code";
...
import { createAgentSession, defineTool } from "@eaonlabs/eaon-code";
...
```typescript
import type { InlineExtension } from "@eaonlabs/eaon-code";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@eaonlabs/eaon-code";
