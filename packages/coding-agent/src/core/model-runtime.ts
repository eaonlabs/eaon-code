...
import * as builtinProviderCatalog from "@eaonlabs/eaon-ai/providers/all";
...
/** Configured Eaon AI Models collection used by coding-agent and SDK consumers. */
export class ModelRuntime implements Models {
	...
	login(providerId: string, type: AuthType, interaction: AuthInteraction): Promise<Credential> {
		...
		return this.enqueueCredentialOperation(providerId, signal, async () => {
			...
			if (this.modelNetworkEnabled && !signal.aborted) {
				...
				this.updateModelSnapshot();
				await this.refreshProviderAvailability(providerId, signal).catch(() => {});
			}
			return credential;
		});
	}
	...
	async refresh(options: ModelsRefreshOptions = {}): Promise<ModelsRefreshResult> {
		...
		// Older Eaon AI builds used a different ModelsStore return contract.
