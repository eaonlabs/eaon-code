...
import { getSystemMessageText } from "@eaonlabs/eaon-ai";
...
export function buildSystemPromptSections(input: BuildSystemPromptOptions): SystemPromptSections {
	...
	if (customPrompt) {
		...
		promptSections.preamble =
			"You are an expert coding assistant operating inside Eaon Code, a coding agent harness. You help users by reading files, executing commands, editing code, and writing new files.";
		...
		promptSections.docs = `Eaon Code documentation (read only when the user asks about the agent itself, its SDK, extensions, themes, skills, or TUI):
...
- When working on agent topics, read the docs and examples, and follow .md cross-references before implementing
- Always read these .md files completely and follow links to related docs (e.g., tui.md for TUI API details)`;
	}
