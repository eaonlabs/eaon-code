...
import { APP_NAME, getProjectConfigDir } from "../config.ts";
...
function formatProjectTrustPrompt(cwd: string): string {
	return `Trust project folder?\n${cwd}\n\nThis allows ${APP_NAME} to load ${basename(getProjectConfigDir(cwd))} settings and resources, install missing project packages, and execute project extensions.`;
}
