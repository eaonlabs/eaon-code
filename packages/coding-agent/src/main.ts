...
import { builtInExtensionPaths, builtInExtensions, hideBundledExtensions } from "./extensions/index.ts";
...
export async function main(args: string[], options?: MainOptions) {
	...
	const offlineMode =
		...
		isTruthyEnvFlag(process.env.EAON_CODE_OFFLINE) ||
	...
	if (await handlePackageCommand(args, { extensionFactories })) {
		...
		if (process.platform === "win32" && exitCode === 0 && args[0] === "update") {
			...
			// runs during teardown; let successful `eaon-code update` drain naturally instead.
	...
	const extensionPaths = [...builtInExtensionPaths, ...(resolvedExtensionPaths ?? [])];
	...
	// Eaon Code reads user-authored themes, so it opts into full validation before any theme loads.
