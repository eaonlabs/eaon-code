...
import { getSupportedThinkingLevels, type Model, type Transport } from "@eaonlabs/eaon-ai";
import {
	Container,
	getCapabilities,
			...
			},
			{
				...
				submenu: (currentValue, done) => {
					const categoryFor = (category: string) =>
						category === "light" ? getLightThemeNames() : getDarkThemeNames();
					const initialCategory = isLightTheme(currentValue) ? "light" : "dark";
						...
						[
							{
								...
								description: "Choose a theme list.",
								options: () => [
									{
										value: "dark",
										label: "Dark themes",
								...
								preselect: () => initialCategory,
							},
							{
								key: "theme",
								title: (context) => `${context.category === "light" ? "Light" : "Dark"} themes`,
								description: "Select a theme.",
								options: (context) => themeItems(categoryFor(context.category), currentValue),
								preselect: (context) => {
									const themes = categoryFor(context.category);
									return themes.includes(currentValue) ? currentValue : themes[0];
								},
							},
						],
						(selections) => {
							if (selections.theme) done(selections.theme);
						},
						() => done(),
					);
				},
