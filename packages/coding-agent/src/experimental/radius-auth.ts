...
import { DEFAULT_RADIUS_GATEWAY, normalizeRadiusGatewayUrl } from "@eaonlabs/eaon-ai/providers/radius-config";
...
export class RadiusRelayAuthResolver {
	...
	async resolve(options: {
		...
		if (options.required) {
			throw new Error("Radius authentication is required; start Eaon Code and run /login radius, then retry");
		}
		return undefined;
	}
