 ...
 * the root: the type forbids it. `p` and `m` may target the root when the tracked value
 * is an array. A root `p` that replaces its entire target is normalised to `r`/`s` at
 * flush time; operation shape is otherwise not canonical for array mutations.
 *
...
const spliceItems = (target: unknown[], index: number, remove: number, items: JsonValue[]): JsonValue[] => {
	...
	const removed = Reflect.apply(Array.prototype.slice, target, [index, index + remove]) as JsonValue[];
	const newLength = oldLength - remove + items.length;
	if (items.length > remove) {
		target.length = newLength;
		Reflect.apply(Array.prototype.copyWithin, target, [index + items.length, index + remove, oldLength]);
