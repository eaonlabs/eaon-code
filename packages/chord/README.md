...
`@eaonlabs/chord`. Import public types and general runtime APIs from the
package root. Context constants and functions live in
`@eaonlabs/chord/context` because their generic names should not pollute
the root API.
...
Import the standalone delta primitive from `@eaonlabs/chord/delta`:
...
`@eaonlabs/chord/bundler` uses esbuild to turn ESM or TypeScript application
...
```ts
import { createFacetBundleLoader } from "@eaonlabs/chord/node";
