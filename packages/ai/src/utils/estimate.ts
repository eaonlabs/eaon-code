...
export function estimateContextTokens(context: TranscriptContext | readonly Message[]): ContextUsageEstimate {
	...
	if (usageInfo) {
		...
		const toolDefinitions = new Map<string, Tool>();
		const activatedToolNames = new Set<string>();
		const declaredAfterUsage = new Set<string>();
		for (let i = 0; i < messages.length; i++) {
			const message = messages[i];
			if (message.role === "system") {
				for (const tool of message.toolsAdded ?? []) {
					toolDefinitions.set(tool.name, tool);
					if (i > usageInfo.index) declaredAfterUsage.add(tool.name);
				}
			} else if (i > usageInfo.index && message.role === "toolResult") {
				for (const name of message.addedToolNames ?? []) activatedToolNames.add(name);
			}
		}
		const activatedTools = [...activatedToolNames]
			.filter((name) => !declaredAfterUsage.has(name))
			.map((name) => toolDefinitions.get(name))
			.filter((tool): tool is Tool => tool !== undefined);
		trailingTokens += estimateToolsTokens(activatedTools);
