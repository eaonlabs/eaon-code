...
```bash
npm install @eaonlabs/eaon-ai
```

TypeBox exports are re-exported from `@eaonlabs/eaon-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@eaonlabs/eaon-ai';
...
import { openrouterProvider } from '@eaonlabs/eaon-ai/providers/openrouter';
import { amazonBedrockProvider } from '@eaonlabs/eaon-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@eaonlabs/eaon-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@eaonlabs/eaon-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@eaonlabs/eaon-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@eaonlabs/eaon-ai';
...
```typescript
import { validateToolCall, type Tool } from '@eaonlabs/eaon-ai';
...
```typescript
import { builtinImagesModels } from '@eaonlabs/eaon-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@eaonlabs/eaon-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@eaonlabs/eaon-ai';
import { openAICompletionsApi } from '@eaonlabs/eaon-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@eaonlabs/eaon-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@eaonlabs/eaon-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@eaonlabs/eaon-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@eaonlabs/eaon-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@eaonlabs/eaon-ai';
import { anthropicProvider } from '@eaonlabs/eaon-ai/providers/anthropic';
import { openaiProvider } from '@eaonlabs/eaon-ai/providers/openai';
import { googleProvider } from '@eaonlabs/eaon-ai/providers/google';
...
- Importing `@eaonlabs/eaon-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@eaonlabs/eaon-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@eaonlabs/eaon-ai/api/bedrock-converse-stream.lazy';
...
```bash
npx @eaonlabs/eaon-ai login              # interactive provider selection
npx @eaonlabs/eaon-ai login anthropic    # login to specific provider
npx @eaonlabs/eaon-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@eaonlabs/eaon-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@eaonlabs/eaon-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@eaonlabs/eaon-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@eaonlabs/eaon-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@eaonlabs/eaon-ai`.
