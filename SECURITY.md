...
This document explains the security model behind Eaon Code and where its boundaries are.

In general Eaon Code is a coding agent that runs locally within the security boundary
...
Eaon Code treats the local user account and files writable by that account as inside
the same trust boundary as the Eaon Code process itself. If an attacker can modify files
under the user's home directory, workspace, shell startup files, environment, or
Eaon Code configuration, they can generally influence Eaon Code or other local developer tools.
Reports that depend on such prior local write access are not security
vulnerabilities unless they demonstrate how Eaon Code grants that write access or crosses
an operating-system privilege boundary.

Eaon Code relies on users installing trustworthy extensions and loading trustworthy
skills and only using the agent within trusted repositories. This is because files
...
repository code are in scope as well as Eaon-operated infrastructure
on `eaon.dev` and `github.com/eaonlabs`.

## Out Of Scope

- Local code execution or sandboxing behavior (Eaon Code intentionally does not have a sandbox)
...
- Public internet exposure of an Eaon Code installation
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  profiles, or dotfile managers, unless the report shows how Eaon Code itself grants
  that access.
...
Eaon Code configuration file cause the agent to execute commands, load attacker-controlled tools,
...
affected and that the issue is reachable through Eaon Code. For exposed-secret reports,
include evidence that the credential is owned by Eaon or grants access to
Eaon-operated infrastructure or services.
